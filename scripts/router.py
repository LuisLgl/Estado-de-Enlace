...
class HelloSender:
    ...
    def __init__(self, router_id: str, interfaces: list[dict[str, str]], neighbors: dict[str, str], interval: int = 10, PORTA: int = 5000):
        ...
        Args:
            ...
            interfaces (list[dict[str, str]]): Lista de dicionários representando as interfaces de rede
            neighbors (dict[str, str]): Dicionário com os roteadores vizinhos conhecidos
            interval (int, opcional): Tempo de intervalo para o envio periódico dos pacotes HELLO
            PORTA (int, opcional): Porta UDP onde o roteador irá escutar os pacotes
        """
...
class LSASender:
    ...
    def __init__(self, router_id: str, neighbors_ip: dict[str, str], neighbors_cost: dict[str, int], interfaces: list[dict[str, str]], lsdb: LSDB, interval: int = 30, PORTA: int = 5000):
        ...
        Args:
            router_id (str): Identificador único do roteador
            neighbors_ip (dict[str, str]): Dicionário com IPs dos vizinhos
            neighbors_cost (dict[str, str]): Dicionário com custos para os vizinhos
            interfaces (list[dict[str, str]]): Lista de interfaces de rede
            lsdb (LSDB): Banco de dados de estado de enlace
            interval (int, opcional): Intervalo para envio de pacotes LSA
            PORTA (int, opcional): Porta UDP para escutar pacotes
        """
        self._router_id = router_id
...
class Roteador:
    ...
    def __init__(self, router_id: str, PORTA: int = 5000, BUFFER_SIZE: int = 4096):
        ...
        Args:
            router_id (str): Identificador único do roteador
            PORTA (int, opcional): Porta UDP onde o roteador irá escutar os pacotes
            BUFFER_SIZE (int, opcional): Tamanho máximo do buffer de recepção
        """
        self._router_id = router_id
    ...
    def listar_enderecos(self) -> list[dict]:
        ...
        Returns: 
            list[dict]: Lista de dicionários com endereços IP e broadcast
        """
...
class GerenciadorVizinhos:
    ...
    def processar_hello(self, pacote: dict, sender_ip: str):
        """
        Processa um pacote HELLO, reconhecendo vizinhos diretos
    ...
    def processar_lsa(self, pacote: dict, sender_ip: str):
        """
        Processa o pacote LSA, atualizando a LSDB
    ...
    def verificar_quedas(self, intervalo_hello: int = 10, tolerancia: int = 3):
        """
        Verifica periodicamente se algum vizinho deixou de enviar pacotes HELLO

        Args:
            intervalo_hello (int, opcional): Intervalo esperado entre pacotes HELLO
            tolerancia (int, opcional): Quantidade de intervalos sem HELLO antes de declarar o roteador como inativo
        """
        while True:
            agora = time.time()
            roteadores_caidos = [
...
if (__name__ == "__main__"):
    ...
    def print2(string: str):
        """
        Função para exibição de uma mensagem, padronizando o formato com o nome do container
        """
        with print_lock:
